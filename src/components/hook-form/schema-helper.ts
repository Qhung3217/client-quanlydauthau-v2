...
export const schemaHelper = {
  ...
  file: (props?: { message?: string; required?: boolean }) =>
    zod.custom<File | string | null>().transform((data, ctx) => {
      ...
      if (!hasFile) {
        if (props?.required !== undefined && !props?.required) return null;
   ...
   */
  files: (props?: { message?: string; minFiles?: number; require?: boolean }) =>
    zod.array(zod.custom<File | string>()).transform((data, ctx) => {
      const minFiles = props?.minFiles ?? 2;

      if (props?.require && !data.length) {
        ctx.addIssue({
          code: zod.ZodIssueCode.custom,
          message: props?.message ?? 'Files is required!',
        });
      } else if (props?.require && data.length < minFiles) {
        ctx.addIssue({
          code: zod.ZodIssueCode.custom,
